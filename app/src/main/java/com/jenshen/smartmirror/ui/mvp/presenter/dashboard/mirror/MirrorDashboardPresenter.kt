...
class MirrorDashboardPresenter @Inject constructor(private val mirrorApiInteractor: MirrorApiInteractor,
    ...
    fun enablePrecipitation(enable: Boolean) {
        if (enable) {
            val updaterForWidget: CurrentWeatherUpdater = widgetFactoryManager.getUpdaterForWidget(WidgetKey(WidgetInfo.CURRENT_WEATHER_WIDGET_KEY)) as CurrentWeatherUpdater
